        ...
        <div class="flex items-center gap-5">
            <div class="h-fit">
                <a href="{{ route('dashboard') }}"
                    class="bg-gray-500 text-white p-3 rounded-lg text-sm font-semibold hover:bg-gray-600 transition cursor-pointer sm:w-auto">
    ...
    <div>
        ...
        <div class="bg-white shadow dark:bg-gray-800 rounded-b-lg p-4 sm:p-6">
            ...
            <div class="space-y-3 md:hidden">
                    ...
                    <div class="border rounded-lg p-4 shadow-sm bg-gray-50 dark:bg-gray-700">
                        <div class="flex justify-between mb-2">
                            <span class="text-sm font-semibold text-gray-700 dark:text-gray-200">
                                {{ $index + 1 }}. {{ $posttest->title }}
                            </span>
                            <span class="text-xs text-gray-500">{{ $posttest->duration }} menit</span>
                        </div>

                        <div class="mb-2">
                            @if ($posttest->progres === 'Belum Dikerjakan')
                                <span class="text-yellow-600 font-semibold text-xs">Belum Dikerjakan</span>
                            @elseif ($posttest->progres === 'Nilai di Bawah 60')
                                <span class="text-red-600 font-semibold text-xs">Nilai di Bawah 60
                                    ({{ $posttest->score }})
                                </span>
                            @else
                                <span class="text-green-600 font-semibold text-xs">Selesai ({{ $posttest->score }})</span>
                            @endif
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            @else
                                <span class="bg-red-500 text-white text-xs px-2 py-1 rounded-full">Tidak Aktif</span>
                            @endif
                        </div>
                        ...
                        <div>
                            @if (!$posttest->status)
                                <span
                                    class="block w-full bg-gray-400 text-white text-xs text-center px-3 py-1 rounded cursor-not-allowed">
                                    Tidak Tersedia
                                </span>
                            @else
                                @if ($posttest->progres === 'Belum Dikerjakan')
                                ...
                                @elseif ($posttest->progres === 'Nilai di Bawah 60')
                                    <span
                                        class="block w-full bg-red-400 text-white text-xs text-center px-3 py-1 rounded cursor-not-allowed">
                                        Nilai anda di bawah 60
                                    </span>
                                @else
                                    <a href="{{ route('post-test.result', $posttest->result_id) }}"
                                        class="block w-full bg-gray-600 text-white text-xs text-center px-3 py-1 rounded hover:bg-gray-700">
                                        Lihat Hasil
                                    </a>
                                @endif
                            @endif
                ...
                @empty
                    <p class="text-center text-gray-500 text-sm">Tidak ada post test tersedia.</p>
                @endforelse
            </div>
        </div>
    </div>
