            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($trainer as $item)
                        @php
                            $no = $trainer->firstItem() + $loop->index;
                        @endphp
                        <tr>
                            ...
                            <td class="px-6 py-4 text-sm text-center flex gap-2">
                                <a href="{{ route('trainer.show', $item->id) }}"
                                    class="w-full px-3 py-1 bg-yellow-500 text-white text-xs rounded hover:bg-yellow-600">
                                    Lihat Detail
                                </a>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-4 text-sm text-center text-gray-800">Belum ada admin</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mt-6 gap-2">
            ...
            @else
                <div class="text-sm text-gray-600 dark:text-gray-300 p-2">
                    Menampilkan
                    @if ($trainer->total() > 0)
                        {{ $trainer->firstItem() }} sampai {{ $trainer->lastItem() }} dari total
                        {{ $trainer->total() }}
                        hasil
                    @else
