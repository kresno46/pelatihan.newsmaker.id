            ...
            <div class="flex items-center" x-data="{ open: {{ $errors->any() ? 'true' : 'false' }} }" x-cloak>
                ...
                <div x-show="open" x-transition @keydown.escape.window="open=false"
                    ...
                    <div class="relative w-full max-w-lg bg-white dark:bg-gray-800 rounded-xl shadow-xl">
                        ...
                        <form action="{{ route('apuppt.posttest.update', $session) }}" method="POST" class="p-5">
                            @csrf @method('PUT')
                            <div class="space-y-4">
                                <div>
                                ...
                                <div class="w-full flex flex-col gap-2">
                                    ...
                                    <select name="apuppt_pt_scope" id="apuppt_pt_scope" {{ $forcedRole ? 'disabled' : '' }}
                                        class="w-full border rounded p-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 @error('apuppt_pt_scope') border-red-500 dark:border-red-400 @enderror">
                                        <option value="">Pilih PT</option>
                                        @foreach ($ptOptions as $pt)
                                            <option value="{{ $pt }}" {{ old('apuppt_pt_scope', $session->apuppt_pt_scope) === $pt ? 'selected' : '' }}>{{ $pt }}</option>
                                        @endforeach
                                    </select>
                                    @if ($forcedRole)
                                ...
                                <div class="w-full flex flex-col gap-2">
                                    ...
                                    <select name="tipe" id="tipe"
                                        ...
                                        <option value="APUPPT"
                                            {{ old('tipe', $session->tipe) == 'APUPPT' ? 'selected' : '' }}>APUPPT
                                        </option>
                                    </select>
                                    @error('tipe')
                                        <p class="text-red-600 dark:text-red-400 text-sm mt-1">{{ $message }}</p>
    ...
    <div class="mt-6 p-6 bg-white dark:bg-gray-800 rounded-xl shadow-md" x-data="{
            ...
            <div class="space-y-2">
                @foreach ($session->questions as $q)
                    ...
                    edit = {
                      ...
                      action: '{{ route('apuppt.question.update', $q) }}'
                    };
                  ...
                  ">
                                    ...
                                    <button type="button" class="text-red-600 text-sm hover:underline"
                                        @click="
                    openDelete = true;
                    del = { action: '{{ route('apuppt.quiz.delete', $q) }}' };
                  ">
                                        Hapus
                                    </button>
                                </div>
        ...
        <div x-show="openQuestion" x-transition @keydown.escape.window="openQuestion=false"
            ...
            <div class="relative w-full max-w-2xl bg-white dark:bg-gray-800 rounded-xl shadow-xl">
                ...
                <form action="{{ route('apuppt.question.store', $session) }}" method="POST" class="p-5">
                    @csrf
                    <div class="space-y-4">
                        <div>
